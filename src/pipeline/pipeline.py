...
from ingestion.weather_api import fetch_weather, fetch_weather_forecast
from utils.db import get_engine, ensure_schema, ensure_predictions_table, ensure_weather_forecast_table
...
def ingest_weather_forecast(engine, forecast_date: str) -> None:
    for city, info in CITIES.items():
        try:
            raw = fetch_weather_forecast(info["lat"], info["lon"], days=7)
            df = pd.DataFrame(raw["hourly"])
            df = df.dropna(subset=["time"])
            df["target_date"] = pd.to_datetime(df["time"]).dt.date
            df["city"] = city
            df["forecast_date"] = forecast_date
            df = df.drop(columns=["time"]).groupby(["target_date", "city", "forecast_date"]).mean().reset_index()
            with engine.begin() as conn:
                conn.execute(text("""
                    DELETE FROM raw.weather_forecast
                    WHERE city = :city AND forecast_date = :forecast_date
                """), {"city": city, "forecast_date": forecast_date})
                df.to_sql("weather_forecast", conn, schema="raw", if_exists="append", index=False)
            print(f"{city}: weather forecast written for {forecast_date}")
        except Exception as e:
            print(f"{city}: ERROR — {e}")
...
if __name__ == "__main__":
    ...
    ensure_weather_forecast_table(engine)

    last_weather = get_last_date(engine, "weather")

    ingest_air_quality(engine, today.strftime("%Y-%m-%d"))

    for date_str in date_range(last_weather, today):
        ingest_weather(engine, date_str)

    ingest_weather_forecast(engine, today.strftime("%Y-%m-%d"))
